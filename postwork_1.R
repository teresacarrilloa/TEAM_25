...
tbl.goles<-table(goles)
addmargins(tbl.goles)
str(tbl.goles)
tbl.goles
...
tbl.por.goles<-prop.table(x = tbl.goles, margin = NULL)
tbl.por.goles

tbl.goles.fhtg<-table(goles.fhtg)
tbl.fhtg<-prop.table(x = tbl.goles.fhtg, margin = NULL)
tbl.fhtg
class(tbl.fhtg)
#tbl.fhtg.sum<-addmargins(tbl.fhtg)
#tbl.fhtg.sum
#str(tbl.fhtg.sum)
#df.fthg.sum<-as.data.frame(tbl.fhtg.sum)
df.fthg<-as.data.frame(tbl.fhtg)
str(df.fthg)

df.fthg.prob<-mutate(df.fthg,probabilidad=Freq*100)
str(df.fthg.prob)
df.fthg.prob

filter(df.fthg.prob,goles.fhtg!="0")#El FACTO viene en formato CHARACTER por lo que hay que usar operadores de caracter y no lógicos

sum(select(filter(df.fthg.prob,goles.fhtg!="0"),probabilidad))#PROBABILIDAD DE QUE FHTG anote X goles
...
df.fthg.prob<-mutate(df.fthg,probabilidad=frecuencia*100)#Obtención de probabilidades
df.fthg.prob
prob.FTHG<-sum(select(filter(df.fthg.prob,goles!="0"),probabilidad))#PROBABILIDAD DE QUE FHTG anote X goles
